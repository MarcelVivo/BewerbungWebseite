...
const { next: NEXT_STOP } = neighboringDockingStops(ANCHOR);
...
export default function ProblemDockingStation() {
  ...
  useEffect(() => {
    ...
    if (enabled) scrollToDockingStation(ANCHOR);
    ...
    <>
      ...
      {editorEnabled && (
        <div
          ...
          style={{ left: `${ringPosition.x + config.objectX}%`, top: `${ringPosition.y + config.objectY}%` }}
          aria-hidden="true"
        >
          <i />
          <span>OBJECT</span>
      ...
      {editorEnabled && (
        <aside ref={calibrationPanel.panelRef} style={calibrationPanel.panelStyle} className={styles.problemDockCalibration} aria-label="Problem-Dockingstation kalibrieren">
          ...
          <label>Y <input type="range" min="10" max="90" step=".05" value={ringPosition.y} onChange={(event) => updateDockRingPosition(ANCHOR, { y: Number(event.target.value) })} /><output>{ringPosition.y.toFixed(2)}</output></label>
          ...
          {NEXT_STOP && (
            <footer>
              <button type="button" onClick={() => { window.location.href = `?dock-editor=${NEXT_STOP.anchor}`; }}>
                NÄCHSTE STATION: {NEXT_STOP.label} →
              </button>
            </footer>
          )}
        </aside>
