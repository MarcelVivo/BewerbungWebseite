...
import savedDock from './marketing-dock.json';
...
const STORAGE_KEY = 'ms-marketing-dock-v1';
const clamp = (value: number, min: number, max: number) => Math.min(max, Math.max(min, value));

export default function MarketingDockingStation() {
  ...
  const calibrationPanel = useDraggableCalibrationPanel('ms-marketing-dock-panel-v1');
  ...
  useEffect(() => {
    ...
    const enabled = editor === 'marketing' || editor === '5';
    ...
    } catch { /* Keep the visual available without persistence. */ }
  }, []);

  useEffect(() => {
    if (!editorEnabled) return;
    try { window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...config, x: ringPosition.x, y: ringPosition.y })); } catch { /* Keep calibration available. */ }
  ...
  const saveToWebsite = async () => {
    setSaveState('saving');
    try {
      const response = await fetch('/api/docking-config?station=marketing', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...config, x: ringPosition.x, y: ringPosition.y }) });
  ...
  return (
    <>
      ...
      {editorEnabled && (
        <aside ref={calibrationPanel.panelRef} style={calibrationPanel.panelStyle} className={styles.systemDockCalibration} aria-label="Marketing-Dockingstation kalibrieren">
          <header onPointerDown={calibrationPanel.startDrag}><strong>MARKETING DOCK</strong><span>CALIBRATION · DRAG</span></header>
          ...
          <footer><button type="button" onClick={() => { setConfig(DEFAULT_DOCK); updateDockRingPosition(ANCHOR, { x: DEFAULT_DOCK.x, y: DEFAULT_DOCK.y }); }}>RESET</button><button type="button" className={styles.problemDockSave} onClick={saveToWebsite} disabled={saveState === 'saving'}>{saveState === 'saving' ? 'SPEICHERT…' : saveState === 'saved' ? 'GESPEICHERT ✓' : saveState === 'error' ? 'FEHLER' : 'IN WEBSITE SPEICHERN'}</button></footer>
        </aside>
      )}
    </>
  );
