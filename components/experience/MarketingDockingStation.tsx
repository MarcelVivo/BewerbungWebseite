...
export default function MarketingDockingStation() {
  ...
  useEffect(() => {
    ...
    try { window.localStorage.setItem(STORAGE_KEY, JSON.stringify(config)); } catch { /* Keep calibration available. */ }
  ...
  const saveToWebsite = async () => {
    setSaveState('saving');
    try {
      const response = await fetch('/api/docking-config?station=marketing', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(config) });
  ...
  return (
    <>
      <div
        ...
        style={{ left: `${config.x}%`, top: `${config.y}%`, width: `${config.width}%`, '--dock-rotation': `${config.rotation}deg`, '--dock-tilt': `${config.tilt}deg` } as CSSProperties}
      ...
      {editorEnabled && (
        <aside ref={calibrationPanel.panelRef} style={calibrationPanel.panelStyle} className={styles.systemDockCalibration} aria-label="Marketing-Dockingstation kalibrieren">
          ...
          <footer><button type="button" onClick={() => setConfig(DEFAULT_DOCK)}>RESET</button><button type="button" className={styles.problemDockSave} onClick={saveToWebsite} disabled={saveState === 'saving'}>{saveState === 'saving' ? 'SPEICHERT…' : saveState === 'saved' ? 'GESPEICHERT ✓' : saveState === 'error' ? 'FEHLER' : 'IN WEBSITE SPEICHERN'}</button></footer>
        </aside>
      )}
    </>
  );
